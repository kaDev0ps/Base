...
pg_cmd()
{
	(
		src_connection_env;
		set -x;	docker exec -it $Container psql -v ON_ERROR_STOP=1 -U $DB_USER -d $DB_NAME -c "\pset pager off" -c "$Cmd";
		return $?;
	);
        local Result=$?;
        return $Result;
...
pg_cmd2()
{
	(
		src_connection_env;
		set -x;	docker exec -it $Container psql -U $DB_USER -d $DB_NAME -c "\pset pager off" $Cmd;
#		-v ON_ERROR_STOP=1 
		return $?;
	);
        local Result=$?;
...
pg_sql_file()
{
	(
		src_connection_env;
		cat $SQLTextFile | (set -x; docker exec -i $Container psql -v ON_ERROR_STOP=1 -U $DB_USER -d $DB_NAME; return $?); #docker exec -it PostgreSQL psql -U $DB_USER -d $DB_NAME -f "$SQLScriptFile";
	);
	local Result=$?;
	return $Result;
}
...
#	(
#		MoreArgs=${@};
#               	set -x; docker exec -it $Container psql $MoreArgs;
#        )
#}
...
pg_backup()
{
	(
		...
		set -x; docker exec -it $Container bash -lc "
			...
			pg_dump --verbose --column-inserts --format=custom -U $DB_USER -d $DB_NAME > /mnt/backup/$BackupFile.pg_restore;
			pg_dump --verbose --column-inserts --format=plain -U $DB_USER -d $DB_NAME > /mnt/backup/$BackupFile.psql;
		";
	)
}
...
echo -e "===> $Action(cont:=$Container, env:=$ConnectionEnv) at $(date):";
...
#        	set -x;
	...
	( shell_cmd )
		...
		MoreArgs="${@:4}";
		(
	        	set -x; docker exec -it $Container bash -lc "$MoreArgs | cat";
	        );
	;;		
